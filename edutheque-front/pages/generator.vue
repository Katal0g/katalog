<template>
  <UContainer class="flex-col p-4 flex gap-4 lg:flex-row">
    <UCard class="w-full h-fit lg:w-1/4">
      ...
      <UForm class="flex flex-col gap-2" :state="formState" @submit="generateContent">
        ...
        <UButton class="mt-2 py-2" block type="submit" color="primary" :loading="loading">
          Générer le contenu
        </UButton>
      </UForm>
    </UCard>
...
async function generateContent() {
  ...
  if (!formState.value.level || !formState.value.subject || !formState.value.title) {
    return;
  }
